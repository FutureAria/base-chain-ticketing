...
숨기지 않고 **측정 가능한 근거와 함께** 적습니다.

### 무엇을 썼나

| 도구 | 쓴 곳 | 근거 |
|---|---|---|
| **Claude (Claude Code)** | 보안 취약점 수정, 테스트 작성, 리팩터링, 문서·주석 작성 | 커밋 트레일러 `Co-Authored-By: Claude` |
| **OpenAI Codex** | 결제(Toss) 연동 구간 작업 | 브랜치 `codex/toss-sy-integration`, `codex/juyoung-before-toss-backup` |

### 얼마나 썼나 — 실측값

팀 원본 저장소(`hsw0914-window/TicketBlockChain`) 기준입니다.

```bash
# 전체 커밋 중 Claude 공동작성 트레일러가 붙은 커밋
$ git log --all --no-merges --format='%H%n%b'     | awk '/^[0-9a-f]{40}$/{h=$0} /Co-Authored-By: Claude/{print h}' | wc -l
52          # 전체 162개 중 32%

# 해당 커밋들이 추가한 줄 수 (이미지·lock 파일 제외)
8,305줄 / 전체 96,097줄  →  8.6%
```

| 구분 | 값 |
|---|---|
| Claude 공동작성 커밋 | 52 / 162건 (**32%**) |
| 해당 커밋의 추가 줄 | 8,305 / 96,097줄 (**8.6%**) |
| 손댄 파일 | 267 / 383개 |

**커밋 수(32%)와 줄 수(8.6%)가 크게 다른 이유** — AI 보조는 주로 **후반 보완 단계**에
집중됐습니다. 초기 기능 구현은 사람이 대량으로 작성했고(커밋 수는 적지만 줄 수가 많다),
후반의 보안 수정·테스트·문서는 건수는 많지만 한 커밋당 줄 수가 적습니다.

### 어디에 많이 썼고, 어디에 안 썼나

| 영역 | 활용도 | 비고 |
|---|---|---|
| 문서·주석 (`.md`, JSDoc, NatSpec) | **높음** | 이 저장소 문서 3,269줄의 상당 부분 |
| 보안 취약점 수정·테스트 | 중간 | 결함 지적과 수정안 검토에 활용, 판단은 사람이 |
| 초기 기능 구현 (화면·API) | 낮음 | 사람이 직접 작성 |
| 스마트 컨트랙트 로직 | 낮음 | 로직은 사람이 작성, **주석(NatSpec)만** AI 보조 |
| 기획·아키텍처 결정 | 없음 | 좌석 확정을 DB에 맡긴 결정 등은 사람 판단 |

### 정량화되지 않은 부분 (정직하게)

- **Codex 사용분은 트레일러가 없어 셀 수 없습니다.** 위 8.6% 에 포함되지 않습니다.
- **다른 팀원의 AI 사용 여부는 확인하지 못했습니다.** 위 수치는 트레일러가 남은 것만 셉니다.
- 트레일러는 특정 도구를 통해 커밋할 때만 붙습니다. **실제 활용 비율은 위 수치보다 높을 수 있습니다.**
